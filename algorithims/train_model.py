...
class RandomForestTrainer:
    ...
    def load_data(self):
        ...
        logging.info("Data loaded and split into train/test sets.")

    def load_best_params(self):
        if not os.path.exists(self.best_params_path) or os.path.getsize(self.best_params_path) == 0:
            logging.warning("Best parameters file is missing or empty.")
            return {}

        try:
            with open(self.best_params_path, 'r') as f:
                best_params = json.load(f)
            logging.info(f"Best hyperparameters loaded: {best_params}")
            return best_params
        except json.JSONDecodeError:
            logging.error("Best parameters file is not valid JSON.")
            return {}

    def train_model(self, best_params):
        self.model = RandomForestRegressor(**best_params, random_state=42)
        self.model.fit(self.X_train, self.y_train)
        logging.info("Model trained with best hyperparameters.")
    ...
    def evaluate_and_save_metrics(self):
        ...
        logging.info(f"Metrics saved to {self.metrics_output_path}")
